/*
				***		ARMA3Alpha HELI PARADROP v0.9 - by SPUn / lostvar	***
	
					Spawns chopper which delivers paradrop group to scene
				
		Calling the script:
		
			default: 		nul = [this] execVM "heliParadrop.sqf";
			...
			custom: 		nul = [spot, side, allowDamage, captive, distance, direction, flyby, fly height, jump distance,
								...
								group, custom init, ID, MP] execVM "heliParadrop.sqf";
...
direction 	= 	"random" or 0-360 (direction where chopper comes from, use quotes with random!) 					DEFAULT: "90"
flyby	=	true/false		(true = chopper just flies thru target, false = stays still while dropping units)		DEFAULT: true
fly height	= number 		(how high chopper flies)																DEFAULT: 150
jump distance =	number 		(how many meters before target location units starts jumping out of heli)				DEFAULT: 250
group size	= number 		(how many units is in para drop group)													DEFAULT: 12
...
smokes	=	true/false		(will units throw cover smokes (on 10m height))											DEFAULT: true
...
target 	= 	patrol target 	(patrolling target for infantry group, options:											DEFAULT: "respawn_vehicle_west"
...
skills 		= 	"default" 	(default AI skills) 																	DEFAULT: "0.8"
...
EXAMPLE: 	nul = [player, 2, false, true, 1000, "random", true, 500, 200, 6, 1, 50, true, false, true, true, player, false, 0.75, nil, nil, 1,false] execVM "heliParadrop.sqf";
*/
